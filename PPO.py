...
class PPO:
    def __init__(self,env, actor_path = None, critic_path= None, trial = None):
    ...
    def _init_hyperparameters(self):
        ...
        if self.trial:
             self.n_updates_per_iteration = self.trial.suggest_int("n_updates_per_iteration", 5, 15)
             self.clip = self.trial.suggest_float("clip", 0.1, 0.4)
             self.lr = self.trial.suggest_float("lr", 1e-4, 1e-2, log=True)
             self.num_minibatches = self.trial.suggest_int("num_minibatches", 5, 20)
             self.ent_coef = self.trial.suggest_float("ent_coef", 0.01, 0.1)
             self.max_grad_norm = self.trial.suggest_float("max_grad_norm", 0.1, 1)
             self.lam = self.trial.suggest_float("lam", 0.9, 0.99)
             self.target_kl = self.trial.suggest_float("target_kl", 0.01, 0.1)
             self.exploration_start = self.trial.suggest_float("exploration_start", 0.5, 0.9)
             self.exploration_end = self.trial.suggest_float("exploration_end", 0.01, 0.1)
             self.exploration_decay = self.trial.suggest_int("exploration_decay", 3000, 10000)
    ...
    def rollout(self,epsilon_threshold=0.0):
            ...
            prehealth = 0 # Tracks player health before taking action
            while t < self.timesteps_per_batch:
                ...
                for ep_t in range(self.max_timesteps_per_episode):
                   ...
                   #Reward Shaping Constants
                    BOSS_DAMAGE_REWARD = 0.1
                    PLAYER_DAMAGE_PENALTY = -0.05
                    NO_DAMAGE_REWARD = 0.01
                    BOSS_HEALTH_REWARD = 0.001
                    PROXIMITY_REWARD_SCALE = 0.002
                    ...
                    if didbosshealthchange != 0:
                        rew +=  (BOSS_DAMAGE_REWARD * didbosshealthchange)
                    ...
                    rew += (BOSS_HEALTH_REWARD * (obs['boss_max_hp']-obs["boss_hp"])/1000)
                    ...
                    rew += proximity_reward * PROXIMITY_REWARD_SCALE
    ...
    def learn(self, total_timesteps):
         #line 2 from the PP0 algo
            ...
            mean_episode_reward = statistics.mean(self.episode_rewards)
